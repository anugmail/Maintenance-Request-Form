...
const PHASE_TH = {
  ...
  inspection:  'เฟส 2 · ตรวจสภาพก่อนซ่อม',
  maintenance: 'เฟส 3 · ดำเนินการบำรุงรักษา',
...
const SAMPLE = {
  ...
  'v.rigBrand':    () => `${MYD.rigLabelOf(sampleV)} ${sampleV.rigBrand}`,
  'v.rigModel':    () => sampleV.rigModel,
  'v.truckBrand':  () => sampleV.truckBrand || '—',
  'v.truckModel':  () => sampleV.truckModel || '—',
  'v.assetCode':   () => sampleV.assetCode,
  'v.serialNo':    () => sampleV.serialNo,
  'v.hcNo':        () => sampleV.hcNo,
  'v.plateFull':   () => MYD.plateFull(sampleV),
  'v.status':      () => MYD.STATUS_LABELS[sampleV.status] || sampleV.status,
  'v.province':    () => sampleV.province,
  'v.statusNote':  () => MYD.statusNote(sampleV) || '— ไม่มีข้อควรระวัง —',
...
const SRC_LABELS = {
  ...
  'v.truckBrand': 'รถ · ยี่ห้อรถยนต์', 'v.truckModel': 'รถ · รุ่นรถยนต์',
  'v.assetCode': 'รถ · รหัสครุภัณฑ์', 'v.serialNo': 'รถ · หมายเลข S/N', 'v.hcNo': 'รถ · หมายเลข HC',
  'v.plateFull': 'รถ · ทะเบียน (มีตัวย่อจังหวัด)',
  'v.province': 'รถ · จังหวัด', 'v.statusNote': 'รถ · ข้อควรระวังของสถานะ', 'v.bucket': 'รถ · ไตรมาสที่จัดไว้',
  ...
  screens: [
    ...
    { id: 's1', group: 'issue', no: '1', title: 'ชื่อแผน + จัดรถเข้าไตรมาส', real: 'plan-new.html', asks: [], sections: [
      ...
      { id: 's1-veh', title: 'ตารางรถในเขต (ภาค → เขต → กางออก)', kind: 'table', fields: [
        ...
        f('plate',    'ทะเบียน', 'v.plateFull'),
        f('assetCode','รหัสครุภัณฑ์', 'v.assetCode'),
        f('type',     'ประเภท', 'v.vehicleType'),
        f('rigBrand', 'ยี่ห้ออุปกรณ์ยก (กระเช้า/เครน)', 'v.rigBrand'),
        f('truckBrand','ยี่ห้อ/รุ่นรถยนต์', 'v.truckBrand'),
        f('detail',   'ปุ่มเปิดรายละเอียดรถ (10 ฟิลด์ตามแบบฟอร์มตรวจสภาพ)', ''),
        f('province', 'จังหวัด', 'v.province'),
        f('owner',    'หน่วยงานเจ้าของรถ', 'v.ownerDept'),
        f('status',   'สถานะรถ (6 แบบ)', 'v.status'),
        f('reason',   'ข้อควรระวังของสถานะ (ไม่ได้ห้ามเลือก)', 'v.statusNote'),
    ...
    { id: 'ph1b', group: 'phase', no: '1b', title: 'เบิก/จัดหาอะไหล่ + แผนเดินทาง (หลายใบ)', real: 'index.html', sections: [
      ...
      { id: 'ph1b-veh', title: 'รถในแต่ละใบ', kind: 'table', fields: [
      ...
      // ✅ ปิดแล้ว 17 ส.ค. 2569 — เจ้าของงานสั่ง "ตอนสร้างแผนการเดินทาง สามารถเลือกจ้าง vendor
    ...
    { id: 'ph2', group: 'phase', no: '3', title: 'ดำเนินการบำรุงรักษา', real: null, sections: [
    ...
    { id: 'ph3', group: 'phase', no: '2', title: 'ตรวจสภาพก่อนซ่อม', real: 'index.html', sections: [
