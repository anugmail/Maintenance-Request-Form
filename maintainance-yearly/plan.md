...
⚠️ ตอนนั้นแตกเฉพาะใน skeleton — `PHASES` ใน `app.js` ยังเป็น 5 เฟส ไม่ได้แตะ
(**อัปเดต 21 ส.ค. 2569:** `PHASES` แตกจริงแล้วเป็น 6 เฟส — แต่จุดตัดคนละจุดกับ `ph1a`/`ph1b` ด้านล่าง
ดูหัวข้อ "รอบแก้ 21 ส.ค. 2569" ท้ายไฟล์)
...
## รอบแก้ 10–11 ส.ค. 2569 — ขั้นยืนยันรถ + แผนเดินทางหลายใบ (เฟส 1 ครบ)

สเปก: [`../docs/superpowers/specs/2026-08-10-confirm-vehicles-design.md`](../docs/superpowers/specs/2026-08-10-confirm-vehicles-design.md)
แผน: [`../docs/superpowers/plans/2026-08-10-confirm-vehicles.md`](../docs/superpowers/plans/2026-08-10-confirm-vehicles.md)

**ลำดับใหม่ของเฟส 1** — `ยืนยันรถ → เบิก/จัดหาอะไหล่ → แผนเดินทาง → ทวน + ยืนยัน`

### ขั้น 1 · ยืนยันรถเข้าร่วมแผน (ใหม่ทั้งขั้น)

- `plan.confirm` — สถานะรายคัน (`pending`/`ready`/`notready`) · เหตุผล · จุดนัดรับ · ผู้ตอบ+เวลา · `history[]` · `verdict` ของ กบค.
- logic ล้วนใน `mock-yearly.js`: `confirmStatus` · `isVehicleIn` · `confirmResolved` · `confirmSummary` · `confirmLocked` (รับ `today` เป็นพารามิเตอร์ เทสเคส "เลยกำหนด" ได้โดยไม่ต้องแก้เวลาเครื่อง)
- **`verdict` ชนะ `answer` เสมอ** — เขียนเป็นเทสไว้เพราะสลับลำดับ if แล้วพังเงียบ
- หน้า กบค. แบ่งรายการเป็น **ภาค → จังหวัด** (`.wgrp` + `.rzone`) พร้อมสรุปต่อกลุ่ม
- หน้าหน่วยงานเจ้าของรถ `confirm.html` — ตอบรายคัน · ล็อกเมื่อ `travelConfirmed`

### ขั้น 3 · แผนเดินทางหลายใบ

- `plan.travelPlan` (ใบเดียว) → **`plan.trips[]`** · `SCHEMA_VERSION` 7 → 8
- ช่วงเวลาระดับใบ · **วันนัดรายคัน** · การตอบรับระดับ **ใบ × หน่วยงาน**
- ทางลัด "แยกอัตโนมัติตามจังหวัด" เป็นจุดตั้งต้น แล้วแตก/ย้ายรถต่อได้อิสระ
- ถูกปฏิเสธ → ใบปลดล็อกให้แก้แล้วส่งใหม่ · ปุ่มเปลี่ยนเป็น "แก้แล้วส่งใหม่"
- dropdown เลือกรถ **จัดกลุ่มตามจังหวัด** (`<optgroup>` + จำนวนคัน)

### บั๊กที่เจอและแก้

| บั๊ก | ผล |
|---|---|
| `plan-new.js` `persist()` เรียกตัวเอง | `RangeError` ทุก handler ⇒ **หน้าออกเลขงานบันทึกอะไรไม่ได้เลย** · หลุดตั้งแต่ 8 ส.ค. อยู่บน main ด้วย · hotfix + deploy แล้ว |
| คลิก stepper ข้ามขั้นได้ | `.locked` เป็นแค่สไตล์ · `goProcSub()` ไม่เช็คอะไร ⇒ กฎ "ยืนยันครบก่อนเบิก" ไม่มีผลจริง · อุดแล้ว (ถอยหลังได้เสมอ เดินหน้าต้องผ่านทุกขั้นก่อน) |
| `.f sp3` | คลาสไม่มีอยู่จริงในระบบ (มีแค่ `.sp2`/`.sp4`) ⇒ ช่องกินแค่ 1 คอลัมน์ |
| `.search` ติดลิสต์ 0px | คอมโพเนนต์ถือ margin ไม่สม่ำเสมอ ⇒ เพิ่ม `.stack` ในระบบ + เครื่องมือตรวจ |

### เทส

`confirm.test.mjs` · `logic.test.mjs` · `skeleton-data.test.js` **27/27**
browser: `verify-trips` **20** · `verify-cf-grouping` **16** · `verify-admin-cf` **11** · `verify-plan-new-select` **16**
`design-system/check-spacing.js` ผ่าน 8 หน้า · `Diagram/check-mermaid.js` ผัง 7/7

### ยังค้าง

- [x] ~~เฟส 2–5 ยังเป็นหน้าเปล่า~~ — **21 ส.ค. 2569:** เฟส "เบิก/จัดหา + แผนเดินทาง" แยกเป็น 2 เฟสแล้ว (ดูหัวข้อถัดไป) เหลือ **เฟส 4–6 ยังเป็นหน้าเปล่า** — เฟส 4 (ดำเนินการบำรุงรักษา) มีเงื่อนไขต้องเคาะ 5 ข้อใน `plan-skeleton.html` (จอ `ph2`)
- [ ] สายว่าจ้างยังไม่ทำในต้นแบบ (ทำเฉพาะสายตรวจเอง)
- [ ] คันที่ "เลื่อนรอบหน้า" ยังไม่ยกไปแผนปีถัดไปอัตโนมัติ
- [ ] เอกสารพัสดุออกก่อนยืนยันรถ ⇒ ยอดไม่ตรงกับที่เบิกจริง — รอเจ้าของงานเคาะ
- [ ] ใครในหน่วยงานมีสิทธิ์กดตอบ (ตอนนี้พิมพ์ชื่อเอง) · จำนวนวันจริง (ตั้ง 7 ไว้ก่อน)

---

## รอบแก้ 21 ส.ค. 2569 — แยกเฟส "เบิก/จัดหา + แผนเดินทาง" เป็น 2 เฟส

เจ้าของงานสั่งแยกขั้นตอนบนหน้าแผนรายใบ: **"เบิก/จัดหา"** เป็นเฟส 1 · **"แผนเดินทาง"** เป็นเฟส 2 ·
เฟสที่เหลือ (ตรวจสภาพก่อนซ่อม · ดำเนินการบำรุงรักษา · จัดทำรายงาน · คำนวณต้นทุน) เลื่อนจาก 2-5 เป็น 3-6

**`PHASES` ใน `app.js`** ตอนนี้มี **6 เฟส**: เบิก/จัดหา(1) · แผนเดินทาง(2) · ตรวจสภาพก่อนซ่อม(3) ·
ดำเนินการบำรุงรักษา(4) · จัดทำรายงาน(5) · คำนวณต้นทุน(6)

**จุดตัดของ wizard เดิม** (4 ขั้น: ยืนยันรถ → เบิกอะไหล่ → แผนเดินทาง → ทวน+ยืนยัน) อยู่ระหว่างขั้น "เบิกอะไหล่"
กับ "แผนเดินทาง" — ตัว wizard ยัง reuse โครงเดิม แค่แบ่ง sub-stepper เป็น `PROC_STEPS` (2 ขั้น เฟส 1)
กับ `TRAVEL_STEPS` (2 ขั้น เฟส 2) คุมด้วย "master step" (1-4 เดิม) ⇒ ลำดับการทำงานของผู้ใช้ไม่เปลี่ยนเลย

**เกณฑ์จบเฟส** — เฟส 1 จบเมื่อ **ยืนยันรถครบทุกคัน + ส่งคำขอเบิกแล้ว**
(`MYD.confirmResolved(...) && plan.partsRequisitioned`) · เฟส 2 จบเมื่อ `plan.travelConfirmed===true`
⚠️ เฟส 1 ต้องเช็ค `confirmResolved` ด้วย ไม่ใช่ดูแค่ `partsRequisitioned` — แผนตัวอย่าง `plan-seed-2569-002`
มี `partsRequisitioned:true` ทั้งที่ยังยืนยันรถไม่ครบ ถ้าดูธงเดียวจะปลดล็อกเฟส 2 ให้ข้ามขั้นยืนยันรถได้
(ขัดกฎที่เจ้าของงานสั่งไว้ 10 ส.ค. 2569) · มี `travelConfirmed` เป็น fallback ให้ข้อมูลเก่าใน localStorage

**ผลกระทบ:**
- `SEED_PLAN` (`mock-yearly.js`) เปลี่ยน `phase` จาก `procurement` เป็น `travel` — เพราะทั้งเบิกอะไหล่และยืนยัน
  แผนเดินทางเสร็จแล้วในข้อมูลตัวอย่างนี้
- `skeleton-data.js` — `PHASE_TH` เพิ่ม key `travel` (เลขเฟส 1-6) · `GROUP_LABELS.phase` เป็น "6 เฟสปฏิบัติการ"
  · **แยกจอ `ph1c` แผนเดินทาง (หลายใบ) ออกจาก `ph1b`** ให้ตัดจุดเดียวกับของจริง ⇒ กลุ่ม phase เป็น 7 จอ
  (`ph1a` ยืนยันรถ + `ph1b` เบิกอะไหล่ = เฟส 1 · `ph1c` = เฟส 2) รวมทั้งหมด 11 จอ · เลข `no` ของจอเฟสหลังเลื่อนเป็น 3-6
  ⚠️ คนที่เคยแก้โครงไว้ใน localStorage จะยังเห็นของเดิม (migrate ให้ของที่แก้เองชนะเสมอ) — กด "คืนโครงตั้งต้น" ถึงจะเห็นจุดตัดใหม่
- `confirm.js` — ข้อความรอแผนนัด อ้าง "เฟส 2 · แผนเดินทาง" แทน "ขั้นที่ 3"
- `Diagram/01-บำรุงรักษาตามวาระ/` — อัปเดตตาราง/ผัง mermaid ให้ตรง 6 เฟสครบทุกไฟล์ (00 · 02 · 03 · 04 · 05 · 06 + `Diagram/README.md`)
  ชื่อไฟล์คงเดิมเพื่อไม่ให้ลิงก์พัง แต่หัวไฟล์บอกเลขเฟสจริงแล้ว
- `test/verify-proc-steps.js` 🆕 — เทสเบราว์เซอร์ไล่ทั้ง 2 เฟส + gate ทั้งระดับเฟสและระดับขั้น (31 ข้อ)

**บั๊กที่เจอระหว่างไล่เทส (คนละเรื่องกับการแยกเฟส แต่แก้ไปด้วย)** — ปุ่ม "ส่งแผนนัดให้หน่วยงาน"
อัปเดตสดตอนพิมพ์ด้วยเกณฑ์ไม่ครบ (เช็คแค่ `tripReadyToSend`) ⇒ ปุ่มเปิดทั้งที่ยังไม่ได้ระบุพนักงาน กบค.
และไม่ยอมเปิดหลังพิมพ์ชื่อพนักงาน (เพราะช่องชื่อไม่ได้ sync ปุ่ม) — รวมเกณฑ์เป็น `tripSendable()` ที่เดียว
แล้วเรียกทั้งตอน render และตอนพิมพ์ (`syncSendBtn`)

**เพิ่มงาน "เปลี่ยนตัวกรอง" (21 ส.ค. 2569)** — เจ้าของงานสั่งเพิ่มตัวเลือกในคอลัมน์ "งานที่จะทำ" ของตารางรถ
ในใบแผนเดินทาง ⇒ `MYD.TRIP_JOBS` เป็น 3 งาน (เปลี่ยนถ่ายน้ำมันไฮดรอลิก · ตรวจน้ำมันไฮดรอลิก · **เปลี่ยนตัวกรอง**)
· ตั้งต้นติ๊กเฉพาะ 2 งานน้ำมัน ตัวกรองเลือกเพิ่มรายคัน · `tripJobsIncomplete()` ไล่จาก `TRIP_JOBS` แทนการเช็คชื่องานตรงๆ
⇒ ชิปในตาราง · ป้าย + ตัวนับในเฟส 4 (ดำเนินการบำรุงรักษา) · เกณฑ์ "ทุกคันต้องเลือกงานอย่างน้อย 1" ตามให้เองทั้งหมด
**เพิ่มงานใหม่ครั้งหน้า = เติมใน `TRIP_JOBS` ที่เดียว**

**เทสที่รัน (21 ส.ค. 2569)** — `skeleton-data.test.js` 29 ✓ · browser: `verify-proc-steps` **31** ✓ ·
`verify-trips` **23** ✓ (แก้เทสที่ค้างจากตอนย้ายวันนัดรายคันไป `confirm.html` ด้วย) · `verify-cf-grouping` **16** ✓ ·
ผัง mermaid ทั้ง 8 บล็อกใน `Diagram/01-บำรุงรักษาตามวาระ/` parse + render ผ่านบน mermaid 11 · ไม่มี pageerror
...
## รอบแก้ 25 ส.ค. 2569 (รอบ 2) — แผนการเดินทางของ "งานซ่อม" (SC-15)

เจ้าของงานสั่งต่อจากการแยกโมดูล: *"อยากให้มีปุ่มสร้างแผนการเดินทางของการซ่อมด้วย"* ·
*"กดเข้าที่สร้างแผนการเดินทางการซ่อม ก็จะเห็นว่ามีรถที่ต้องออกไปซ่อมกี่คัน"* ·
*"โครงสร้างเหมือนหน้า[แผนเดินทางบำรุงรักษา]ได้เลย ตัดแต่ตรงเลือกไตรมาสออกไป"* · *"เอาปุ่มแยกอัตโนมัติตามจังหวัดออก"*

**เคาะระหว่างทาง**

| เรื่อง | คำตอบ |
|---|---|
| 1 ใบเดินทางครอบกี่ใบแจ้งซ่อม | **รวมหลายใบได้** — ทริปเดียวเก็บงานที่อยู่จังหวัดเดียวกัน |
| ไตรมาส | **ไม่มี** ในสายซ่อม |
| ส่งนัด / ตอบรับ | **เป็นช่วงเหมือนสายบำรุงรักษา** ไม่เปลี่ยน |
| ขั้นขออนุมัติแผนเดินทาง | **ต้องมี** — แต่ยังไม่ทำในรอบนี้ (ดู "ยังค้าง") |
| ขอบเขตรอบนี้ | **เฉพาะจังหวะสร้างแผน จบที่ "ส่งแผนนัด"** |
| **เงื่อนไขรถที่เข้าแผนได้** | **เฉพาะใบที่เลือก "จัดซ่อมที่หน้างาน"** ในหัวข้อ *รูปแบบการซ่อม* — ใบที่เลือก *เข้าซ่อมที่ กบค.* ไม่ต้องเดินทาง จึงไม่เข้าพูล (เคาะ 25 ส.ค. 2569 พร้อม screenshot หน้าจริง) |

**ของที่เพิ่ม**

| ไฟล์ | อะไร |
|---|---|
| `mock-yearly.js` | `SEED_REPAIR_JOBS` 8 ใบ (4 จังหวัด) · `REPAIR_TRIPS_KEY` · `emptyRepairTrip` + helper (`repairTripBlockers`/`Sendable`/`Cost`/`Days`/`Depts`/`unassignedRepairJobs`) |
| `trip-plan.js` | `renderRepairStep1` / `bindRepairStep1` — โครงกล่องเดียวกับสายบำรุงรักษา ตัดไตรมาส + ปุ่มแยกอัตโนมัติ |
| `trip-plan-page.js` | แถบสลับสายงาน (`.seg`/`.sg`) บนหน้าเลือก + หน้า `#repair` |
| `test/verify-repair-trip.js` 🆕 | 23 ข้อ |

**เงื่อนไขเข้าพูล** — `SEED_REPAIR_JOBS` ทุกใบมีฟิลด์ `repairMode`
· `'onsite'` = จัดซ่อมที่หน้างาน ⇒ เข้าพูล (6 จาก 8 ใบ) · `'kbk'` = เข้าซ่อมที่ กบค. ⇒ ไม่เข้า (2 ใบ)
หน้าจอบอกเงื่อนไขไว้ด้วย `note-info` และ**แสดงใบที่ถูกกันออกท้ายหน้าพร้อมเหตุผล** ไม่ใช่ซ่อนเงียบๆ
(`MYD.onsiteRepairJobs()` / `offsiteRepairJobs()` — `unassignedRepairJobs()` กรองผ่าน onsite เท่านั้น)

**แยกตามจังหวัด (25 ส.ค. 2569)** — เจ้าของงานสั่งเพิ่ม: ใบที่ยังไม่ถูกจัดเข้าแผน แสดงเป็น
กล่อง `.rzone` **รายจังหวัด** (เรียงจังหวัดที่มีใบเยอะสุดขึ้นก่อน) + บรรทัดสรุปยอดรายจังหวัดที่หัวหน้า
เพราะทีมช่างมักออกทริปเดียวเก็บงานในจังหวัดเดียวกัน · ⚠️ **ไม่ใช่ปุ่ม "แยกอัตโนมัติ"** ซึ่งสั่งให้ตัดออกไปแล้ว —
อันนี้เป็นการ**จัดกลุ่มให้ดู** คนทำแผนยังเลือกเองว่าจะรวมหรือแยกทริป

**ฟิลด์ที่มีเฉพาะสายซ่อม** — `pickupPoint` (จุดนัดรับรถ · SC-15/AC-3.5) · `crewVehicle` (รถที่ใช้เดินทาง · UC-15.1)
ทั้งคู่**บังคับกรอก** ก่อนส่งแผนนัดได้ · งานที่จะทำมาจาก**อาการในใบแจ้งซ่อม** อ่านอย่างเดียว

### 🐞 บั๊กที่เจอตอนทำ — ลอกแพตเทิร์นผิด

ตอนแรกผูก `input`+`change` แล้วให้ `change` **re-render ทั้งหน้า** ⇒ ค่าที่เพิ่งกรอกหายทีละช่องเงียบๆ
(การวาดใหม่ทับค่าใน DOM ก่อนที่ handler ของช่องถัดไปจะทำงาน — เจอเป็น `crewVehicle` ว่างทั้งที่กรอกแล้ว)

**แพตเทิร์นที่ถูกของโปรเจกต์นี้อยู่ในสายบำรุงรักษาอยู่แล้ว** (`syncSendBtn` · บันทึกทันทีแต่ไม่ re-render
เพราะจะทำโฟกัส/เคอร์เซอร์หลุด) — แก้ให้ตรง: พิมพ์ = บันทึก + sync เฉพาะปุ่มส่งกับกล่องเตือน
· re-render เต็มเฉพาะการเปลี่ยนโครง (เพิ่ม/ลบใบ · เพิ่ม/ลบงาน · เพิ่ม/ลดคน · ส่งแผน)

⚠️ บทเรียน: ก่อนเขียน handler ใหม่ ให้ดูก่อนว่าไฟล์เดียวกันแก้ปัญหานี้ไว้ยังไงแล้ว

### 🎨 design-system — เก็บหนี้ที่ค้างมาก่อน

`audit-usage.js` เฟส 3 จับได้ว่า `trip-plan.html` นิยาม `.wstep`/`.wsteps.sm` ซ้ำใน `<style>` (6 จุด)
ซึ่ง `index.html` ก็ผิดอยู่ก่อนแล้ว (7 จุด) ⇒ **ยกขึ้น `components.css` v0.24** ทั้งชุด เหลือ 0 ทั้งสองไฟล์
· พบว่า README v0.7 เคยบันทึกว่ายก `.wstep.locked` ขึ้นมาแล้วตั้งแต่ 10 ส.ค. **แต่ของจริงยังค้างอยู่ในหน้า**
· บั๊ม `?v=20260825-ds24` ครบ 28 ไฟล์ · เพิ่มแถว sub-stepper ในตารางข้อ 4.1 + changelog ข้อ 8
· เพิ่ม `trip-plan.*` เข้ารายการที่ `audit-usage.js` ไล่ตรวจ (เฟส 3) — ก่อนหน้านี้ไฟล์ใหม่ไม่ถูกตรวจเลย

**ผลตรวจครบชุด** — เทสเบราว์เซอร์ **148 ข้อ ล้ม 0** (`verify-repair-trip` 30 · `verify-trips` 23 เท่าเดิม ·
`verify-trip-plan-page` 21 · `verify-proc-steps` 31 · อีก 3 ไฟล์ 11+16+16) · `skeleton-data` 29 ·
node test 2 ไฟล์ · `verify-tokens` ผ่าน · `check-spacing` ผ่าน · grep hex/emoji ว่างทั้งคู่

### ยังค้าง (รอบนี้)

- [ ] **ขั้นขออนุมัติแผนเดินทาง** ของสายซ่อม — เจ้าของงานเคาะแล้วว่าต้องมี ยังไม่ได้ทำ
      · ผมเสนอวางเป็นขั้นที่ 3 หลังหน่วยงานตอบรับครบ (ค่าใช้จ่ายนิ่งแล้วค่อยขออนุมัติ) **ยังไม่ได้รับการยืนยัน**
- [ ] ฝั่งหน่วยงานเจ้าของรถ **ตอบรับแผนนัดสายซ่อม** (เลือกวันในช่วง / ปฏิเสธ) — สายบำรุงรักษามีแล้วที่ `confirm.html`
- [ ] **ยืนยันแผนเดินทาง** สายซ่อม + หน้าสรุป
- [ ] ใบแจ้งซ่อมยังเป็น `SEED_REPAIR_JOBS` จำลอง — ของจริงต้องดึงจากโฟลว์แจ้งซ่อม พร้อมค่า `รูปแบบการซ่อม` จริง
- [ ] **ยังไม่ได้เคาะว่า section แผนเดินทางต้องไปโผล่ใต้ "รูปแบบการซ่อม" ในหน้าใบแจ้งซ่อมด้วยไหม**
      เจ้าของงานส่ง screenshot หน้าจริงมาพร้อมข้อความ *"section นี้จะแสดงมาด้านล่าง"* —
      รอบนี้ทำเป็น**หน้าพูลรวม**ก่อน (เพราะ 1 ใบเดินทางรวมหลายใบแจ้งซ่อม ซึ่งใส่ในหน้าใบเดียวไม่ลงตัว)
- [ ] `.tbl tfoot .sumrow td` ยังนิยามซ้ำใน `index.html`/`supplies.html` · `plan-new.html` เหลือ 7 จุด (หนี้เก่า ไม่ใช่ของรอบนี้)

---

## รอบแก้ 28 ส.ค. 2569 (รอบ 4) — เลือกรถที่จะดำเนินการก่อนตรวจสภาพก่อนซ่อม

เจ้าของงานสั่ง: *"ปรับให้สามารถเลือกรถที่จะบำรุงรักษาได้ก่อนที่จะเข้าไปตรวจสภาพรถก่อนบำรุง เมื่อกดเปิดดำเนินการที่ไตรมาสนั้น
เพื่อป้องกันการเข้ามาบำรุงรักษารถคันเดียวกัน"* — ถามต่อ 2 ข้อ: (1) ล็อกถาวรไหม → **"แค่ปรับให้มีหน้าเลือกรถก่อนที่จะเข้า
บำรุงรักษาก็พอ"** (ไม่ต้องมีปุ่มคืนรถ) (2) จัดกลุ่มยังไง → **จัดกลุ่มตามใบเดินทาง**

**กลไก** — เข้าไตรมาสครั้งแรก (`route()` เจอ `#planId/Q` ที่ยืนยันแผนเดินทางแล้ว) เช็ค `MYD.quarterOpsPicked(plan,q)`
ถ้ายังไม่เคยเลือก → คั่นด้วยหน้า **"เลือกรถที่จะดำเนินการ"** (`renderQuarterVehiclePick`) ก่อนเข้า stepper 4 เฟส —
จัดกลุ่มรถที่ยืนยันเข้าไตรมาสตาม**ใบเดินทาง** (`.rzone` ต่อใบ + checkbox "เลือกทั้งใบ") ตั้งต้นติ๊กทุกคัน ปลดติ๊กได้ถ้า
จะให้ทีมอื่นดำเนินการแทน · กด "เริ่มดำเนินการ" → `MYD.pickQuarterOpsVehicles` บันทึกลง `plan.opsPicked[q]` **ถาวร**
ครั้งต่อไปเข้าไตรมาสเดิมข้ามหน้านี้ไปที่ stepper ตรงๆ (ไม่ถามซ้ำ ตามที่เจ้าของงานสั่งให้ทำแค่หน้าเดียวพอ ไม่ต้องมีปุ่มแก้)

**ขอบเขต** — ทั้ง 4 เฟสของไตรมาส (ตรวจสภาพก่อนซ่อม/ดำเนินการบำรุงรักษา/จัดทำรายงาน/คำนวณต้นทุน) เปลี่ยนจากอ่าน
"รถทุกคันที่ยืนยันเข้าไตรมาส" (`bucketOf(plan,id)===q`) เป็นอ่าน `MYD.quarterOpsPickedIds(plan,q)` แทนทั้งหมด (7 จุดใน
`app.js`) — รถที่ไม่ได้ติ๊กจะไม่โผล่ในทั้ง 4 เฟสนี้เลย (แต่ยังนับรวมในหัวข้อ "รถของไตรมาสนี้ N คัน" เพื่อบอกบริบท)

**ของที่เพิ่ม** — `mock-yearly.js`: `quarterOpsPicked`/`quarterConfirmedIds`/`quarterOpsPickedIds`/`pickQuarterOpsVehicles`
· `app.js`: `renderQuarterEntry` (จุดเข้าเดียว คั่นหน้าเลือกรถ) · `groupQuarterVehiclesByTrip` · `renderQuarterVehiclePick`/
`bindQuarterVehiclePick` (เช็คบ็อกซ์ทั้งหมด/ทั้งใบ/รายคัน อิง `.chk`/`.rzone`/`.tbl` เดิม ไม่มี component ใหม่)

**ผัง** — `Diagram/01-บำรุงรักษาตามวาระ/00-ภาพรวม.md` (โหนดใหม่ก่อนเข้า S3 ตรวจสภาพก่อนซ่อม) +
`04-เฟส3-ตรวจรับ.md` (โหนด `P`/`PA`/`PB` ต้นผัง + แก้ตาราง "หน้าจอที่ทำแล้ว") + `Diagram/README.md` แถว 04 — mermaid 7/7 parse ผ่าน

**ตรวจแล้ว** — `verify-proc-steps` 45 ✓ (เพิ่ม 3 ข้อใหม่เช็คหน้าเลือกรถ) · `verify-trips`/`verify-trip-plan-page`/
`verify-cf-grouping`/`verify-plan-new-select`/`verify-repair-trip` เท่าเดิมทุกไฟล์ (ล้มเฉพาะ 2+3 ข้อเดิมที่เป็น seed-data
drift อยู่ก่อนแล้วบน `main` — เทียบกับ `git stash` แล้วล้มเหมือนกัน ไม่ใช่ของรอบนี้) · node test 3 ไฟล์ผ่าน ·
grep hex/emoji ว่างทั้งคู่ · `audit-usage` ไม่มีจุดใหม่ (checkbox ใช้ `.chk`/`.rowChk`/`.rzone-allchk` แพตเทิร์นเดียวกับ
`plan-new.js` เดิม) · ขับ Chrome จริงเห็นหน้าเลือกรถ + ยืนยันเลือกแค่บางคันแล้วเห็นว่าเฟสตรวจสภาพขึ้นเฉพาะคันที่เลือกจริง

### ยังค้าง (รอบนี้)

- [ ] **ไม่มีปุ่มแก้รายชื่อที่เลือกไว้ทีหลัง** — เลือกครั้งเดียวถาวรตามที่เจ้าของงานสั่งให้ทำแค่หน้าเดียวพอ ถ้าติ๊กผิด
      ตอนนี้ต้องแก้ผ่านข้อมูลตรงๆ (`plan.opsPicked`) ไม่มี UI คืนรถกลับ "ยังไม่มีใครรับ"
- [ ] **รถที่เพิ่มเข้าไตรมาสทีหลัง (หลังกดเริ่มดำเนินการไปแล้ว) จะไม่โผล่ในหน้าจอ 4 เฟสเลย** — เพราะ `opsPicked`
      บันทึกครั้งเดียวไม่ไล่ตามรถใหม่ที่ยืนยันเข้ามาสมทบทีหลัง (เคสน่าจะไม่เกิดบ่อยเพราะยืนยันแผนเดินทางไปแล้วก่อนเข้าไตรมาสนี้ได้)
- [ ] **ยังไม่มีการ "ล็อก" ข้ามเซสชัน/ผู้ใช้จริง** — ต้นแบบเป็น static HTML + `localStorage` เดียว (ไม่มีแบ็กเอนด์/บัญชีผู้ใช้จริง)
      หน้าเลือกรถแค่แสดงกลไก UX ไว้ ยังพิสูจน์การกันชนกันของสองผู้ใช้จริงพร้อมกันไม่ได้จนกว่าจะมีระบบหลังบ้านจริง
