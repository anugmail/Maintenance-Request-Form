...
## 11. รอบแก้ที่ 3 — เลือก component ให้ถูกตัว (25 ส.ค. 2569)

รอบก่อนไม่ crash แล้ว แต่ Page header ยังตั้ง property ไม่ได้ 5 ตัว
(`Badge#` `Actions#` `Divider#` `Breakpoint` `Title#`)

### สาเหตุ — แคตตาล็อกโกหก เพราะรวม component คนละตัวที่ชื่อเหมือนกัน

`Page header` มี **4 ตัว**ในไฟล์ property คนละชุด:

| ตัว | property |
|---|---|
| 1, 3 | `Page` · `Status` |
| 2, 4 | `Badge#` · `Supporting text#` · `Actions#` · `Divider#` · `Title#` · `More actions#` · `Copy action#` · `Back action#` · **`Breakpoint`** |

ตัวกลั่นรวมทั้งหมดเป็นชุดเดียว ⇒ แมปอ้าง `Page`+`Status`+`Title#` พร้อมกัน
ซึ่ง **ไม่มีตัวไหนมีครบ** · แล้วปลั๊กอินก็หยิบ instance ตัวแรกที่เจอ ซึ่งบังเอิญเป็นตัวที่มีแค่ `Page`/`Status`

### แก้ 3 ชั้น

1. **ตัวกลั่น** — เก็บ `propertySets[]` ชุด property ของแต่ละตัวจริงไว้ด้วย ไม่ใช่รวมก้อนเดียว
2. **validator** — ตรวจว่า *"มี component ตัวจริงสักตัวที่มี property ครบทุกตัวที่แมปอ้าง"*
   ไม่ใช่แค่ "มีอยู่ในชุดรวม" ⇒ **จับ `.page-title-row` ได้ทันที** ว่าอ้างข้ามตัว
3. **ปลั๊กอิน** — เก็บผู้สมัคร **ทุกตัว**ที่ชื่อนั้น (แยกด้วยลายเซ็น property)
   แล้ว `pickCandidate()` เลือกตัวที่ property ตรงกับที่สเปกอยากตั้งมากที่สุด (เทียบด้วยชื่อฐาน)

### แมปที่แก้
`.page-title-row` ตัด `Page`/`Status` ออก เหลือชุดของตัวที่มี `Title#` · ตั้ง `Badge=false`
เพื่อไม่ให้ป้ายสถานะของ instance ต้นแบบติดมา

### เทส 15/15
เพิ่มเคสที่ 5 — วาง **instance หลอก** ชื่อเดียวกันแต่ property คนละชุดไว้**หน้าสุด**
ถ้าปลั๊กอินหยิบตัวแรกมั่วๆ จะเจอตัวหลอกก่อนและเทสตก

> บทเรียน: เทสชุดเดิมผ่าน 13/13 ทั้งที่ของจริงพัง เพราะ mock ให้ component ชื่อไม่ซ้ำกัน
> — mock ที่ง่ายกว่าของจริง ทำให้เทสไม่มีความหมาย
