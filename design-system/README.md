...
|---|---|---|
| 18 ส.ค. 2569 | v0.23 | **รอบไล่ให้ตรงหน้าจริงหลังเจ้าของงานทักว่าไม่ตรง** — ได้ screenshot หน้า **"งานซ่อม"** (เลข `RAM…`) เพิ่มอีกหน้า เทียบแล้วแก้ 4 จุด · **ปุ่ม "ตัวกรอง"** แทน `<select>` เปลือยบนแถบ (`.btn.btn-s` + `filter_list` + `.filter-panel` กางออกมา) · **ป้ายนับเงื่อนไขบนปุ่มโชว์เสมอ รวมตอนเป็น 0** ใช้ `.b-neutral` ตามของจริง · **`.tbl.striped`** แถวคู่พื้น gray-50 (หน้ารายการจริงมีแถบสลับสี ต่างจาก `.dataTable` ที่วัดไว้ตอน v0.18 จึงทำเป็น opt-in) · **วันที่เป็น `DD/MM/พ.ศ.` + เวลาบรรทัดล่าง** ไม่ใช่ชื่อเดือนไทยย่อ · **`.tbl .cell-clip`** ตัดข้อความยาวด้วย … + `title` · บั๊ม `?v=20260818-ds23` ทุกหน้า |
| 18 ส.ค. 2569 | v0.22 | **หน้ารายการ (list page) ครบชุด — ปิดช่องว่างข้อ 9 ที่ค้างมาตั้งแต่ต้น** เจ้าของงานส่ง screenshot หน้าจริง **"งานแจ้งซ่อม" (จัดการเหตุรถเสีย)** มาให้ ⇒ ของที่เคยเข้าไม่ได้เพราะ 401 มีของจริงให้ยกแล้ว · เพิ่ม **`.list-toolbar.split`** + `.lt-search`/`.lt-actions` (ค้นหาซ้าย · ตัวกรอง+CTA ขวา) · **`.tbl th.sortable`** หัวตารางกดเรียง (ทั้งช่องเป็น `button`, `swap_vert`→`arrow_upward`/`arrow_downward`) · **`.tblfoot`** แถบท้ายตาราง (แสดง x ถึง y จาก z + จำนวนแถวต่อหน้า + `.pager`) · **`.filter-panel`** แผงเงื่อนไขที่กางจากปุ่ม "ตัวกรอง" · **`.select-inline`** select ในแถบที่ไม่มี label (ใช้ที่ `.tblfoot`) · **`.tbl .cell-key`** ค่าหลักในเซลล์ · **`.tbl .cell-clip`** ตัดข้อความยาวด้วย … + `title` · **`mock/Maintenance-Request-Form.html` เปลี่ยนลิสต์ทั้ง 3 (ของฉัน / คัดแยก กรย. / คิวงาน กบค.) จากการ์ด accordion → ตาราง 7 คอลัมน์** กดแถวไปหน้ารายละเอียดเต็มหน้า (`view-mydet` / `view-krydet` ใหม่ · กบค. ใช้ `view-step` เดิม) ⇒ `.rowtop`/`.vdiv` ในหน้าตายแล้ว ลบทิ้ง · **วันที่ในตารางเป็นตัวเลข `DD/MM/พ.ศ.` + เวลาบรรทัดล่าง** ตามหน้าจริง (เดิมใช้ชื่อเดือนไทยย่อ) · คอลัมน์ "สถานะรถทดแทน" + เลข RAM/RPL ในรูป **ไม่ทำ** เพราะโฟลว์เราไม่มีข้อมูลรองรับ (กติกา no-data-no-ui) · `audit-usage.js` เพิ่มข้อยกเว้น 2 ข้อ (`th.sortable > button` ไม่ต้องใส่ `.btn` · `.search` ใน `.list-toolbar` ไม่ต้องห่อ `.stack` เพราะ toolbar มี margin ของตัวเอง) · บั๊ม `?v=20260818-ds22` ทุกหน้า (รวมหน้าที่ค้าง ds17 มาตั้งแต่ v0.18) |
