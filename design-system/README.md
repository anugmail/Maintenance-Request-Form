...
> **เวอร์ชัน:** v0.14 (13 ส.ค. 2569) · **สถานะ:** ใช้กับทุกหน้าใน prototype แล้ว (24/24)
...
| 3 | เช็คว่าของที่จะทำ **มีในไลบรารี PEA ไหม** ดูตารางความครอบคลุม ก–ง | [SOURCES.md](SOURCES.md) |
| 4 | ถ้ามีในไลบรารี → **ยกค่าจากไลบรารีมาตรงๆ** ห้ามกะเอง | `node design-system/compare-figma.js [ชื่อ]` |
| 5 | ถ้าไม่มีในไลบรารี → ทำได้ แต่ต้องเขียนกำกับว่ามาจากไหน (screenshot หน้าจริง / ของเราเอง) และแจ้งเจ้าของงาน | คอมเมนต์ใน `components.css` + SOURCES.md หมวด ค |
| 6 | เขียนเสร็จ **รันตรวจ 3 ตัวก่อนบอกว่าเสร็จ** | `verify-tokens.js` · `compare-figma.js` · `audit-usage.js [เฟส]` |
...
| แถวรายการ | `.job` > `.sp` (`.no`/`.meta`) (`.sel`) | แถวเรื่อง/รายการในลิสต์ — ใส่ `.sel` เมื่อใช้เป็น**รายการให้เลือก** (เช่น เลือกช่างจากรายชื่อ) |
...
|---|---|---|
| 17 ส.ค. 2569 | v0.19 | เพิ่ม **`.tbl .cell-sub`** — บรรทัดรองในเซลล์ (12/18 gray-500) สำหรับค่าขยายความของค่าหลัก เช่น ยี่ห้อใต้ชนิดรถ · หน่วยงานใต้จังหวัด · เหตุผลใต้ป้ายสถานะ · ก่อนหน้านี้เขียน `style="font-size:12px;color:…"` ซ้ำในหน้า ⇒ ขนาดหลุดสเกล · **ต่างจาก `.sub`** ที่เป็นคำโปรยใต้หัวข้อ section (14px + margin ล่าง) ห้ามใช้สลับกัน · เพิ่ม **`.tbl tr.vrow-blocked`** — แถวที่เลือกไม่ได้ (ตัวอักษร gray-500 · ช่องติ๊กจาง 50%) ยังอ่านออกเพื่อให้เห็นว่ามีรถคันนี้อยู่และทำไมถึงเลือกไม่ได้ · เกิดจากตารางเลือกรถเข้าแผนที่ต้องปิดรถสถานะซ่อมอยู่/หมดสภาพ/โอนย้าย/รอจำหน่าย · บั๊ม `?v=20260817-ds19` ทุกหน้า |
