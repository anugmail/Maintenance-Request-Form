...
/* ---------- Stepper: ขั้นตอนของ mock (ปรับชื่อ/ลำดับ/เปิดปิด/หัวข้อ จาก admin) ----------
   wizard: 3 ขั้นของฟอร์มแจ้งซ่อม (key ผูกกับ logic — vehicle/symptom/parts)
   kbk: 5 ป้ายสถานะงานซ่อม กบค. (label ล้วน — rename/reorder ได้) */
/* 7 ก.ย. 2569: เจ้าของงานส่งสเปกเต็มชุดจากดีไซน์จริง — ยุบเหลือ 4 ขั้น
   (สถานะรถ + รูปแบบการซ่อม + จุดที่แจ้ง + งบ + ผู้แจ้งเหตุ ย้ายมารวมขั้น 1 ·
    ขั้นสถานที่/เหตุการณ์ตัดเหลือ ไมล์/ชั่วโมงเครื่องจักร/รูป/หมายเหตุ ·
    ขั้นสรุป+เลือกผู้อนุมัติถูกตัด — ส่งอนุมัติตรงจากขั้นอะไหล่ ผู้อนุมัติ = หัวหน้าหน่วยงานอัตโนมัติ) */
/* 8 ก.ย. 2569: เจ้าของงานสั่งรวมขั้น "รายละเอียดเหตุการณ์" กับ "อาการเสีย" เป็นขั้นเดียว
   ⇒ เหลือ 3 ขั้น · ขั้นที่รวมมี 2 บล็อกในหน้าเดียว (อาการเสีย → รายละเอียดเหตุการณ์)
   label = ชื่อขั้นที่ขึ้นบนหัวฟอร์ม · heading = หัวข้อบล็อกแรกในหน้า (บล็อกที่ 2 ตายตัว)
   คีย์ 'info' ถูกถอดออก — config เก่าที่ยังมีคีย์นี้ถูกกรองทิ้งเองที่ normSteps() */
const WIZARD_DEFAULT=[
  {key:'vehicle', label:'ข้อมูลยานพาหนะและผู้แจ้ง', heading:'ข้อมูลยานพาหนะและผู้แจ้ง', on:true},
  {key:'symptom', label:'อาการเสียและรายละเอียดเหตุการณ์', heading:'อาการเสีย', on:true},
  {key:'parts',   label:'อะไหล่ที่แนะนำ',   heading:'อะไหล่ที่แนะนำ',      on:true}
];
...
const defaults={
  ...
  parts:[
    ...
    {sym:'HYD-02',fit:'all',code:'PM-2210',name:'ปั๊มไฮดรอลิกเกียร์',need:1,unit:'ตัว',stock:0,stockAlt:0,wh:'กบค. สนญ.',icon:'compress',price:18500},
    ...
    {sym:'ELEC-02',fit:'all',code:'SN-3310',name:'เซ็นเซอร์ load cell',need:1,unit:'ตัว',stock:0,stockAlt:0,wh:'กบค. สนญ.',icon:'sensors',price:12500},
    ...
    {sym:'ELEC-02',fit:['SK17A'],code:'AC-1103',name:'เซ็นเซอร์ระดับกระเช้า Aichi',need:1,unit:'ตัว',stock:0,stockAlt:2,wh:'กบค. สนญ.',icon:'sensors',price:8900},
    ...
    {sym:'BOOM-02',fit:['TM-ZE304'],code:'TD-2204',name:'ชุดเบรกวินช์ Tadano',need:1,unit:'ชุด',stock:0,stockAlt:1,wh:'กบค. สนญ.',icon:'settings_backup_restore',price:12400},
    ...
    {sym:'HYD-02',fit:['URV554'],code:'UN-3302',name:'ปั๊มไฮดรอลิก Unic URV554',need:1,unit:'ตัว',stock:0,stockAlt:0,wh:'กบค. สนญ.',icon:'compress',price:21000},
...
const SEED_QUEUE3=[
  {no:'MTD-690719-051',plate:'81-2345 นครราชสีมา',model:'Hino FM8J 6 ล้อ + เครน Tadano TM-ZE304',org:'กฟภ. เขต ฉ.3 นครราชสีมา',target:'เครน Tadano TM-ZE304',driver:{name:'พขร. สุชาติ แก้วมณี',tel:'044-242-100'},owner:{name:'กฟจ.นครราชสีมา',tel:'044-242-100'},
  ...
  {no:'MTD-690719-052',plate:'82-6789 ขอนแก่น',model:'Isuzu FTR + กระเช้า Aichi SK17A',org:'กฟภ. เขต ฉ.1 ขอนแก่น',target:'กระเช้า Aichi SK17A',driver:{name:'พขร. อนุชิต ศรีสุข',tel:'043-221-100'},owner:{name:'กฟจ.ขอนแก่น',tel:'043-221-100'},
  ...
  {no:'MTD-690719-053',plate:'83-1122 กรุงเทพมหานคร',model:'Mitsubishi Fuso FI + เครน Unic URV554',org:'สำนักงานใหญ่ (กบค.)',target:'เครน Unic URV554',driver:{name:'พขร. ประเสริฐ มั่นคง',tel:'02-590-9999'},owner:{name:'สำนักงานใหญ่ (กบค.)',tel:'02-590-9999'},
   syms:['ไฟเตือน load sensor ขึ้น'],sev:'หยุดใช้งาน',photos:3,desc:'ไฟเตือนขึ้นค้าง ยกงานไม่ได้',
   booked:[],ordered:[{name:'เซ็นเซอร์ load cell',qty:1,unit:'ตัว'}],reason:'อะไหล่หมดคลัง รอของเข้า',
   phase:'pending',wi:0,history:[{t:T+' 13:05',label:'ส่งเรื่องถึง กบค. — รอรับเรื่อง'}]}
];

const SEED_MIXED6=[
  {no:'MTD-690719-054',plate:'80-5566 เชียงใหม่',model:'Hino XZU กระบะยกสูง',org:'กฟภ. เขต น.1 เชียงใหม่',target:'ตัวรถ',driver:{name:'พขร. มานพ ใจงาม',tel:'053-241-000'},owner:{name:'กฟจ.เชียงใหม่',tel:'053-241-000'},
  ...
  SEED_QUEUE3[0],
  {no:'MTD-690718-055',plate:'82-6789 ขอนแก่น',model:'Isuzu FTR + กระเช้า Aichi SK17A',org:'กฟภ. เขต ฉ.1 ขอนแก่น',target:'กระเช้า Aichi SK17A',driver:{name:'พขร. วีระ ทองสุข',tel:'043-272-200'},owner:{name:'กฟส.บ้านไผ่',tel:'043-272-200'},
   syms:['ปั๊มไฮดรอลิกเสียงดัง/แรงดันตก'],sev:'ปานกลาง',photos:1,desc:'แรงดันตกตอนยกโหลดเกินครึ่ง',
   booked:[{name:'ไส้กรองไฮดรอลิก',qty:2,unit:'ชิ้น'}],ordered:[{name:'ปั๊มไฮดรอลิกเกียร์',qty:1,unit:'ตัว'}],reason:'ปั๊มต้องเปลี่ยนทั้งลูก รอของเข้า',
   inspIn:INSP_IN,phase:'work',wi:1,
   history:[{t:'18 ก.ค. 2569 10:05',label:'ส่งเรื่องถึง กบค. — รอรับเรื่อง'},{t:'18 ก.ค. 2569 15:40',label:'กบค. รับเรื่องซ่อม'},{t:T+' 09:30',label:'ตรวจสภาพรับรถเข้าซ่อม (ขาเข้า) — น้ำมัน ½ · ตำหนิรอบตัวรถ 1 จุด'},{t:T+' 10:00',label:'สถานะ: รอสั่งอะไหล่ (ปั๊มไฮดรอลิกเกียร์)'}]},
  {no:'MTD-690718-056',plate:'81-2345 นครราชสีมา',model:'Hino FM8J 6 ล้อ + เครน Tadano TM-ZE304',org:'กฟภ. เขต ฉ.3 นครราชสีมา',target:'เครน Tadano TM-ZE304',driver:{name:'พขร. บุญส่ง ดีใจ',tel:'044-412-300'},owner:{name:'กฟส.สีคิ้ว',tel:'044-412-300'},
  ...
  {no:'MTD-690718-057',plate:'83-1122 กรุงเทพมหานคร',model:'Mitsubishi Fuso FI + เครน Unic URV554',org:'สำนักงานใหญ่ (กบค.)',target:'เครน Unic URV554',driver:{name:'พขร. ประเสริฐ มั่นคง',tel:'02-590-9999'},owner:{name:'สำนักงานใหญ่ (กบค.)',tel:'02-590-9999'},
  ...
  {no:'MTD-690717-058',plate:'82-6789 ขอนแก่น',model:'Isuzu FTR + กระเช้า Aichi SK17A',org:'กฟภ. เขต ฉ.1 ขอนแก่น',target:'กระเช้า Aichi SK17A',driver:{name:'พขร. อนุชิต ศรีสุข',tel:'043-311-400'},owner:{name:'กฟส.ชุมแพ',tel:'043-311-400'},
